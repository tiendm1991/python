    ...
    for i in range(n - 2, -1, -1):
        for j in range(i + 1, n):
            ...
            for k in range(i + 1, j):
                dpMin[i][j] = min(dpMin[i][j], calculate(dpMin[i][k - 1], dpMin[k][j], op[k-1]), calculate(dpMin[i][k], dpMin[k + 1][j], op[k]))
                dpMax[i][j] = max(dpMax[i][j], calculate(dpMax[i][k - 1], dpMax[k][j], op[k-1]), calculate(dpMax[i][k], dpMax[k + 1][j], op[k]))
